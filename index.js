  ...
  },
  {
    ...
    axios.all([
      ...
      .then(axios.spread((res1, res2) => {
        const html = res1.data.login+".html";
       
        

      let getData = {...res1.data};
          getData.starQuantity = res2.data.length;


          fs.writeFile(html, generateHTML({...getData, ...{color}}), function(err) {
            console.log("write", res1.data.name +".html")
            if(err) {
              throw err;
            }
            const readHtml = fs.readFileSync("./" +html, "utf8");

            pdf.create(readHtml, options).toFile(`./${res1.data.name}.pdf`, function(err, res){
